...
from env.task_context import ContinuousSelector, NetGenTaskContext
...
parser.add_argument('--rollouts', default=250, type=str, help='Number of rollouts')
...
tasks = NetGenTaskContext(
    base_id=[11, 21, 22, 31, 32],
    inflow=ContinuousSelector(100, 500, 3),
    green_phase=ContinuousSelector(20, 30, 2),
    red_phase=ContinuousSelector(20, 30, 2),
    lane_length=ContinuousSelector(50, 750, 4),
    speed_limit=ContinuousSelector(15, 20, 2),
    offset=ContinuousSelector(1, 1, 1),
    single_approach=[True],
    penetration_rate=ContinuousSelector(args.penetration, args.penetration, 1),
...
for i in range(args.rollouts):
    ...
    if i % args.save_frequency == 0:
        ...
        checkpoint_dir = algo.save(save_dir)
